...
#property description "Calculates lot size based on SL price and risk in USD, then opens a single SELL position with optional TP."
...
void OnStart()
{
   ...
   {
      Alert("Error: For a SELL order, the Stop Loss Price must be above the current Bid Price (",
            bidPrice, ").");
      return;
   }
   ...
   if(normSL <= bidPrice)
   {
      Alert("Error: Normalized SL (", normSL, ") is not above Bid (", bidPrice, ").");
      return;
   }
   ...
   if(minDist > 0.0 && (normSL - bidPrice) < minDist)
   {
      Alert("Error: SL too close. Distance (", DoubleToString(normSL - bidPrice, digits),
   ...
   if(InpSetTP)
   {
      tp = NormalizeDouble(bidPrice - (normSL - bidPrice) * InpRR, digits);
      if(tp <= 0.0)
      {
         Alert("Error: Calculated TP (", tp, ") is invalid (<= 0). Lower the RR ratio.");
         return;
      }
      if(minDist > 0.0 && (bidPrice - tp) < minDist)
      {
         Alert("Error: TP too close. Distance (", DoubleToString(bidPrice - tp, digits),
               ") < broker minimum stops level (", DoubleToString(minDist, digits),
               "). Raise the RR ratio.");
         return;
      }
   ...
   {
      Print("Failed to place SELL order. Result code: ", trade.ResultRetcode(),
